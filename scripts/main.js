...
for (let bigImage of images) {
    ...
    function openLightBox() {
        ...
        if (bigImage.classList.contains('page_picture_down')) {
            bigImage.classList.remove('page_picture_down');
            bigImage.classList.add('down_lightbox');
        }

        if (bigImage.classList.contains('page_picture_top')) {
            bigImage.classList.remove('page_picture_top');
            bigImage.classList.add('top_lightbox');
        }
    }
...
function init() {
    new SmoothScroll(document, 300, 40); // target / speed / smoothness
}
...
function startMovement() {
    ...
    function moveElements(moveX, moveY) {
        ...
        for (let baseitem of moveBasicElem) {
            generateRandom();
            baseitem.style.transform = `translate(${moveDistanceX - random}px,${moveDistanceY + random}px) scale(${getRandomArbitrary(0.99, 1.01)}) rotate(${getRandomArbitrary(-1, 1)}deg)`;
        }
...
setTimeout(() => setInterval(startMovement, 1000), 5000)
